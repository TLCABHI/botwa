...
/******** OWNER NUMBER**********/
const ownerNumber = ["919074329903@s.whatsapp.net","919497534004@s.whatsapp.net"] //ganti nomor lu
/************************************/
           ...
           return false
			...
			switch(command) {
            	...
            	case 'mining':
                      ...
                      reply(`because you are our owner from the bot team sending ${one}Xp got you`)
                      }else{
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (args.length < 1) return client.sendMessage(from, 'Language code required!!', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Which text to make voice? :v?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('What do you want to add a genie to??')
					if (args[0].startsWith('08')) return reply('Use koGunakande mas')
					try {
           ...
           case 'setname':
                ...
                client.sendMessage(from, 'Succes, Rename Group', text, {quoted: mek})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝘆𝗮𝗵 @${mentioned[0].split('@')[0]} your admin is uninstalled🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
